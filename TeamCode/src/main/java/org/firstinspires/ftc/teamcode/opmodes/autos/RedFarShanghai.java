 ...
 * Mirrored from BlueFarShanghai.
 * 7 paths with curves and specific shooting positions.
 */
@Autonomous(name = "Red Far Shanghai", group = "Far")
public class RedFarShanghai extends AutoCommandBase {
    ...
    private final Pose parkPose = mirrorPose(new Pose(35.86226203807391, 12.094064949608057, Math.toRadians(90)));

    // Mirrored control points (using Pose with heading=0)
    private final Pose ctrl1 = mirrorCtrl(new Pose(57.245240761478165, 37.088465845464725, 0));
    private final Pose ctrl2 = mirrorCtrl(new Pose(61.76, 27.9, 0));
    private final Pose ctrl3 = mirrorCtrl(new Pose(65.31, 37.09, 0));
    private final Pose ctrl6 = mirrorCtrl(new Pose(35.15341545352744, 32.57334826427771, 0));
    ...
    private static Pose mirrorCtrl(Pose pose) {
        return new Pose(FIELD_WIDTH - pose.getX(), pose.getY(), 0);
    }
    ...
    private void buildPaths() {
        // Get mirrored headings
        double startHeading = startPose.getHeading();
        double shootPose1Heading = shootPose1.getHeading();
        double intakePoseHeading = intakePose.getHeading();
        double shootPose2Heading = shootPose2.getHeading();
        double pushStartHeading = pushStartPose.getHeading();
        double pushEndHeading = pushEndPose.getHeading();
        double shootPose3Heading = shootPose3.getHeading();
        double parkHeading = parkPose.getHeading();

        // Path 1: Start -> Shoot Pose 1 (BezierLine)
        path1_toShootPose = follower.pathBuilder()
                .addPath(new BezierLine(startPose, shootPose1))
                .setLinearHeadingInterpolation(startHeading, shootPose1Heading)
                .build();
        ...
        path2_toIntakePose = follower.pathBuilder()
                .addPath(new BezierCurve(
                        ...
                        ctrl3,
                        intakePose))
                .setLinearHeadingInterpolation(shootPose1Heading, intakePoseHeading)
                .build();
        ...
        path3_toShootPose2 = follower.pathBuilder()
                ...
                .setLinearHeadingInterpolation(intakePoseHeading, shootPose2Heading)
                .build();
        ...
        path4_toPushStart = follower.pathBuilder()
                ...
                .setLinearHeadingInterpolation(shootPose2Heading, pushStartHeading)
                .build();
        ...
        path5_toPushEnd = follower.pathBuilder()
                ...
                .setLinearHeadingInterpolation(pushStartHeading, pushEndHeading)
                .build();
        ...
        path6_toShootPose3 = follower.pathBuilder()
                .addPath(new BezierCurve(
                        ...
                        ctrl6,
                        shootPose3))
                .setLinearHeadingInterpolation(pushEndHeading, shootPose3Heading)
                .build();

        // Path 7: Shoot Pose 3 -> Park (BezierLine)
        path7_toPark = follower.pathBuilder()
                .addPath(new BezierLine(shootPose3, parkPose))
                .setLinearHeadingInterpolation(shootPose3Heading, parkHeading)
                .build();
    }
}
